      ...
      <Tabs defaultValue="dashboard" className="w-full space-y-6">
        <TabsList className="grid w-full grid-cols-2 gap-2 bg-transparent md:grid-cols-4">
          {showcaseItems.map((item) => (
            <TabsTrigger
              key={item.value}
              value={item.value}
              className="data-[state=active]:bg-[#FF4500] data-[state=active]:text-white px-2 py-1 text-sm rounded-md"
            >
              {item.title}
            </TabsTrigger>
          ))}
        ...
        {showcaseItems.map((item) => (
          <TabsContent key={item.value} value={item.value} className="mt-6">
            <div className="overflow-hidden rounded-md border bg-gradient-to-b from-muted/50 to-muted p-0.5 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
              <motion.div
